...
class VanMoofBikeBleClient:
    ...
    async def _async_read_candidate(self, device: BLEDevice) -> BikeState:
        ...
        return BikeState(
            ...
            alarm_state=_coerce_int(topics.get(Topic.ALARM)),
            backup_code=_stringify_value(topics.get(Topic.BACKUP_CODE)),
            lock_state_alt=_map_locked(topics.get(Topic.LOCK_STATE_ALT)),
